...
st.set_page_config(page_title="Fronteira Eficiente — Otimizador de Portfólio (Pro)", layout="wide")
...
# Header / Badges
# =====================
st.title("Fronteira Eficiente Pro — Cálculos Diários, Regras por Classe e Exportáveis")
st.caption("Dados reais (BCB + yfinance), janela 1–3 anos, μ personalizável, Σ com shrinkage, limites por classe, fronteira eficiente, "
           "regras setoriais (RDMercado/Crédito) e exportações de relatórios.")
...
def apply_class_overrides_from_prices(prices: pd.DataFrame, mu_ann: pd.Series, Sigma_ann: pd.DataFrame, win_key: str, start_date: str) -> Tuple[pd.Series, pd.DataFrame]:
    ...
    # Build returns for mixing computation
    rets_all = to_returns(prices, method="log")
    # 1) RDMercado synthetic: 0.3*IRFM + 0.7*SELIC
    # Use column 'RDMercado' as IRFM proxy; if not present, try IRFM11.SA
    try:
        ...
        if s_irfm is not None and s_selic is not None:
            ...
            # recompute covariances with the mixed series
            for a in cols:
    ...
    # 2) CreditoTrad using IDA-DI stats
    ...
    # 3) CredEstrut: sigma = sigma(CreditoTrad)+1pp ; mu = SELIC+1pp
...
if "sigma_override" not in st.session_state and _prices_preview is not None:
    # Suggested vols per earlier discussion
    sugg = {
...
use_sigma_override = st.sidebar.checkbox("Usar Σ fixa (substitui apenas a diagonal com sigmas abaixo)", value=False)
...
# Config persistence
st.sidebar.subheader("Salvar / Carregar Config")
def _download_blob(name: str, data: bytes):
    b64 = base64.b64encode(data).decode()
    st.sidebar.markdown(f"<a download='{name}' href='data:application/octet-stream;base64,{b64}'>Baixar {name}</a>", unsafe_allow_html=True)

if st.sidebar.button("Baixar CONFIG (JSON)") and _prices_preview is not None:
    cfg = {
        "mapping": st.session_state.get("real_map", {}).copy() if isinstance(st.session_state.get("real_map"), pd.DataFrame) else None,
        "bounds": st.session_state.get("bounds_ranges", {}),
        "sigma_override": st.session_state.get("sigma_override", {})
    }
    if isinstance(cfg["mapping"], pd.DataFrame):
        cfg["mapping"] = cfg["mapping"].to_dict(orient="list")
    _download_blob("config.json", json.dumps(cfg, indent=2).encode())

cfg_file = st.sidebar.file_uploader("Carregar CONFIG (JSON)", type=["json"], key="cfg_up")
if cfg_file is not None:
    try:
        cfg = json.loads(cfg_file.getvalue().decode())
        if cfg.get("mapping"):
            df = pd.DataFrame(cfg["mapping"])
            st.session_state["real_map"] = df
        if cfg.get("bounds"):
            st.session_state["bounds_ranges"] = {k: tuple(v) for k,v in cfg["bounds"].items()}
        if cfg.get("sigma_override"):
            st.session_state["sigma_override"] = cfg["sigma_override"]
        st.sidebar.success("Config carregada.")
    except Exception as e:
        st.sidebar.error(f"Falha ao carregar config: {e}")
...
with tab_est:
    ...
    # Optional Sigma diagonal override
...
with tab_opt:
    ...
    if objective == "Sharpe Máximo":
        w = None if infeasible else optimize_max_sharpe(mu_ann, Sigma_ann, rf, lb, ub)
        if w is None:
        ...
        w = None if infeasible else optimize_min_var_no_target(Sigma_ann, lb, ub)
        if w is None:
...
with tab_report:
    ...
    rep_bytes = json.dumps(report, indent=2).encode()
    st.download_button("Baixar Relatório (JSON)", data=rep_bytes, file_name="relatorio_execucao.json")
...
_badge("Exportações prontas", "#0ea5e9")
