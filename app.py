...
st.caption("Dados reais (BCB + yfinance), retornos diários anualizados por 252, μ personalizável, limites por classe via sliders e fronteira eficiente.")

# =====================
# Utils
# =====================
def read_uploaded_csv(uploaded_file):
    """Safely read uploaded CSV; compatible with Streamlit uploader handling multiple reads."""
...
def detect_format(df: pd.DataFrame) -> pd.DataFrame:
    """Detect long format (date,ticker,price) and pivot to wide; otherwise treat as wide."""
...
def build_real_dataset(mapping: pd.DataFrame, start: str = "2018-01-01") -> pd.DataFrame:
    """Monta painel de preços diário a partir de um mapeamento (Classe, Fonte, Identificador).
       Retorna painel com colunas válidas; falhas em st.session_state['__build_failures__'], fallbacks em st.session_state['__fallbacks_applied__'].
    """
    cols = []
    failures = []
...
# =====================
# Class overrides (your business rules)
# =====================
def _apply_variance_override(Sigma: pd.DataFrame, asset: str, target_sigma: float) -> pd.DataFrame:
    """Rescala linha/coluna de 'asset' para var=target_sigma^2, preservando correlações aproximadas."""
...
def apply_class_overrides(rets: pd.DataFrame, mu_ann: pd.Series, Sigma_ann: pd.DataFrame, win_key: str) -> Tuple[pd.Series, pd.DataFrame]:
    """
    Regras:
      - RDMercado = 0.3*IRFM (usa a série atual de 'RDMercado' como IRFM) + 0.7*SELIC  -> recalcula μ e covariâncias com série sintética
      - CreditoTrad: μ/σ do IDA-DI (a partir do print), por janela (1a/2a/3a)          -> override μ e variância (mantendo correlações)
      - CredEstrut: σ = σ(CreditoTrad)+1pp ; μ = SELIC+1pp                              -> override μ e variância
    """
    mu = mu_ann.copy()
    S = Sigma_ann.copy()
    cols = list(rets.columns)

    # μ/σ para IDA-DI por janela (extraído da imagem; anualizados)
    IDADI_SIGMA = {"1a": 0.0204, "2a": 0.0145, "3a": 0.0176}
    IDADI_MU    = {"1a": 0.1335, "2a": 0.1353, "3a": 0.1336}

    # 1) RDMercado sintético (0.3*IRFM + 0.7*SELIC)
    if "RDMercado" in cols and "SELIC" in cols:
        try:
            r_mix = 0.3*rets["RDMercado"] + 0.7*rets["SELIC"]
            mu.loc["RDMercado"] = float(r_mix.mean() * 252)
            for a in cols:
                cov = float(np.cov(r_mix, rets[a], ddof=1)[0, 1] * 252)
                S.loc["RDMercado", a] = cov
                S.loc[a, "RDMercado"] = cov
            S.loc["RDMercado", "RDMercado"] = float(r_mix.var() * 252)
        except Exception as e:
            st.warning(f"Falha ao aplicar mix RDMercado (0.3 IRFM + 0.7 SELIC): {e}")

    # 2) CreditoTrad com IDA-DI (μ/σ por janela)
    if "CreditoTrad" in cols:
        try:
            ...
            st.warning(f"Falha ao aplicar override CreditoTrad (IDA-DI): {e}")

    # 3) CredEstrut: σ = σ(CreditoTrad)+1pp ; μ = SELIC+1pp
    if "CredEstrut" in cols:
        try:
            mu_selic = float(mu.get("SELIC", 0.0))
            mu.loc["CredEstrut"] = mu_selic + 0.01  # +1pp
            if "CreditoTrad" in cols:
            ...
            st.warning(f"Falha ao aplicar override CredEstrut: {e}")
...
if st.session_state.get("use_real_online", False):
    ...
    default_map = pd.DataFrame({
        ...
        "Identificador": [
            "1178",        # SELIC (BCB/SGS)
            "IMAB11.SA",   # NTN-B até 5 anos (ETF)
            "IRFM11.SA",   # prefixados (ETF do IRF-M)
            "IRFM11.SA",   # Crédito Tradicional -> IRFM como base de correlação (μ/σ serão override)
            "IRFM11.SA",   # Crédito Estruturado -> IRFM como base de correlação (μ/σ serão override)
            "BOVA11.SA",   # ações BR
            "^GSPC",       # S&P 500
            "BOVA11.SA"    # multimercado (proxy simples)
        ]
    })
...
st.sidebar.checkbox(
    "Aplicar regras de classe (RDMercado mix, CreditoTrad IDA-DI, CredEstrut +1pp/+1pp)",
    value=True,
...
with tab_est:
    ...
    if st.session_state.get("apply_class_rules", False):
        ...
        mu_ann, Sigma_ann = apply_class_overrides(returns, mu_ann, Sigma_ann, win_key)
...
with tab_opt:
    ...
    if st.session_state.get("apply_class_rules", False):
        win_key = '1a' if st.session_state.get('win_days',504)==252 else ('2a' if st.session_state.get('win_days',504)==504 else '3a')
        mu_ann, Sigma_ann = apply_class_overrides(rets, mu_ann, Sigma_ann, win_key)
...
st.markdown("---")
st.caption("Boas práticas: shrinkage (Ledoit-Wolf) p/ Σ; anualização por 252; janela de 1–3 anos; regras de classe aplicadas opcionalmente; validar ∑Min ≤ 1 ≤ ∑Max.")
