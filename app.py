...
def build_class_constraints(assets: List[str], class_df: pd.DataFrame) -> List[ClassConstraint]:
    """Create list of ClassConstraint from a DataFrame with columns: Ativo, Classe, Min, Max (fractions)."""
    grp = class_df.groupby("Classe")
    out = []
    for cname, sub in grp:
        idxs = [assets.index(a) for a in sub["Ativo"] if a in assets]
        if len(idxs) == 0:
            continue
        # Take min/max for the class: use the first row's Min/Max if uniform; otherwise take min of mins / max of maxes
        min_w = float(sub["Min"].min())
        max_w = float(sub["Max"].max())
        out.append(ClassConstraint(name=cname, indices=idxs, min_w=min_w, max_w=max_w))
    return out
...
def optimize_max_sharpe(mu: pd.Series, Sigma: pd.DataFrame, rf: float, lb: np.ndarray, ub: np.ndarray, classes: Optional[List[ClassConstraint]] = None) -> Optional[np.ndarray]:
    ...
    best_sharpe = -1e18
    for t in targets:
        w = optimize_min_var(mu, Sigma, t, lb, ub, classes=classes)
        if w is None:
            continue
...
with tab_opt:
    ...
    if uploaded is None:
        ...
        if objective == "Sharpe Máximo":
            w = optimize_max_sharpe(mu_ann, Sigma_ann, rf, lb, ub, classes=classes)
            if w is None:
            ...
            w = optimize_min_var(mu_ann, Sigma_ann, target_ret, lb, ub, classes=classes)
            if w is None:
...
with tab_frontier:
    ...
    if uploaded is None:
        ...
        frontier = compute_frontier(mu_ann, Sigma_ann, lb, ub, n_points=30, classes=classes)
        if len(frontier) == 0:
