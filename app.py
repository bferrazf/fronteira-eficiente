...
def build_real_dataset(mapping: pd.DataFrame, start: str = "2018-01-01") -> pd.DataFrame:
    """Monta painel de preços diário a partir de um mapeamento (Classe, Fonte, Identificador).
       Retorna painel com colunas válidas; falhas são reportadas em st.session_state['__build_failures__'].
    """
    cols = []
    failures = []
    for _, row in mapping.iterrows():
        ...
        try:
            if fonte == 'yfinance':
                ...
                # 'anbima' aceita: (a) URL CSV com colunas [data, valor] ou [date, value]; (b) aliases conhecidos
                s = None
                ident_low = ident.lower()
                import requests as _rq
                try:
                    if ident_low.startswith('http'):
                        r = _rq.get(ident, timeout=30)
                        r.raise_for_status()
                        from io import StringIO
                        tmp = pd.read_csv(StringIO(r.text))
                        # Normaliza colunas
                        cols_cand = {c.lower(): c for c in tmp.columns}
                        if 'data' in cols_cand:
                            tmp['date'] = pd.to_datetime(tmp[cols_cand['data']], dayfirst=True, errors='coerce')
                        elif 'date' in cols_cand:
                            tmp['date'] = pd.to_datetime(tmp[cols_cand['date']], errors='coerce')
                        else:
                            raise ValueError('CSV ANBIMA precisa de coluna data/date')
                        if 'valor' in cols_cand:
                            val = pd.to_numeric(tmp[cols_cand['valor']], errors='coerce')
                        elif 'value' in cols_cand:
                            val = pd.to_numeric(tmp[cols_cand['value']], errors='coerce')
                        else:
                            # tenta última coluna numérica
                            num_cols = tmp.select_dtypes(include=['float','int']).columns
                            if len(num_cols)==0:
                                raise ValueError('CSV ANBIMA sem coluna de valores')
                            val = pd.to_numeric(tmp[num_cols[-1]], errors='coerce')
                        df = pd.DataFrame({'date': tmp['date'], 'valor': val}).dropna()
                        df = df.set_index('date').sort_index()
                        s = df['valor']
                    else:
                        # Aliases básicos -> instrução para usar URL
                        # Ex.: 'IDA_DI', 'IDA_IPCA_INFRA'
                        raise ValueError("Para 'anbima', informe um URL CSV em 'Identificador' (ex.: link direto do Data ANBIMA).")
                except Exception as e:
                    failures.append((classe, fonte, ident, f'Falha ANBIMA: {e}'))
                    s = None
                if s is None or s.empty:
                    pass
                else:
                    # Se a série for rentabilidade (%), converte para índice 100
                    # Heurística: se valores estiverem em torno de 0-5, assume % a.m. e trata como taxa;
                    # senão, se já parecer índice (>50), usa direto.
                    if s.dropna().median() < 10:
                        s_idx = _rate_series_to_price_index(s)
                    else:
                        s_idx = s
                    cols.append(s_idx.rename(classe))
            else:
                failures.append((classe, fonte, ident, "Fonte desconhecida. Use 'yfinance', 'bcb' ou 'anbima' (CSV URL)."))
        except Exception as e:
            failures.append((classe, fonte, ident, f'Erro inesperado: {e}'))
    st.session_state['__build_failures__'] = failures
    # ---- Fallbacks automáticos por classe ----
    # Se determinadas classes falharam, tentamos proxies yfinance para não interromper o pipeline.
    # Mapeamento de fallback (classe -> (fonte, identificador))
    fallback_map = {
        "CreditoTrad": ("yfinance", "IMAB11.SA"),   # proxy funcional p/ crédito tradicional
        "CredEstrut":  ("yfinance", "IFIX.SA"),     # proxy p/ crédito estruturado
        "RDMercado":   ("yfinance", "IRFM11.SA"),   # proxy p/ prefixados (curva de juros)
    }
    # Classes já montadas
    ...
    for (classe, fonte, ident, motivo) in list(failures):
        if classe in fallback_map and classe not in assembled:
            ...
            try:
                ...
                # (outros tipos de fallback podem ser adicionados aqui)
            except Exception as e:
...
if st.session_state.get("use_real_online", False):
    ...
    default_map = pd.DataFrame({
        ...
        "Fonte":  ["bcb",   "yfinance","yfinance",  "anbima",      "anbima",    "yfinance","yfinance","yfinance"],
        "Identificador": [
            "1178",
            "IMAB11.SA",
            "IRFM11.SA",
            "https://data.anbima.com.br/feeds/IDA-DI.csv",
            "https://data.anbima.com.br/feeds/IDA-IPCA-INFRA.csv",
            "BOVA11.SA",
            "^GSPC",
            "BOVA11.SA"
        ]
    ...
    st.sidebar.caption("Observação: os links CSV da ANBIMA podem exigir acesso/autenticação e podem mudar. Substitua pelo link atualizado caso apareça falha.")
    ...
    if st.sidebar.button("Baixar & Montar dados"):
        ...
        if panel.empty:
            ...
            # Reportar falhas parciais, se houver
        ...
        # Se vazio, ainda assim reportar falhas
        if panel.empty:
            fails = st.session_state.get('__build_failures__', [])
            if fails:
