    ...
    if uploaded is None:
        ...
        with c_ex2:
            if st.button("Usar exemplo"):
                try:
                    sample = pd.read_csv("dummy_prices.csv")
...
with tab_est:
    ...
    if uploaded is None:
        ...
        st.caption("Opcional: edite os retornos esperados (μ anualizado) usados na otimização. Se não editar, usamos a estimação acima.")
        ...
        df_custom = st.dataframe(st.session_state["custom_mu"].to_frame("mu_ann").style.format({'mu_ann': '{:.2%}'}))

        st.caption("Para editar, abra o menu de três pontos no canto da tabela e selecione **Download as CSV**, edite e reenvie; ou use o campo abaixo.")
        custom_input = st.text_area("Cole aqui pares 'Ativo, μ_anualizado' (por linha), ex.:  AcoesBR, 0.12", value="", height=100)
        if st.button("Aplicar retornos personalizados"):
            try:
                if custom_input.strip():
                    rows = [r.strip() for r in custom_input.strip().splitlines() if r.strip()]
                    custom_series = st.session_state["custom_mu"].copy()
                    for r in rows:
                        parts = [p.strip() for p in r.split(",")]
                        if len(parts) >= 2:
                            k = parts[0]
                            v = float(parts[1])
                            if k in custom_series.index:
                                custom_series.loc[k] = v
                    st.session_state["custom_mu"] = custom_series
                    st.success("Retornos personalizados aplicados.")
                else:
                    st.info("Nada colado; mantendo valores atuais.")
            except Exception as e:
                st.error(f"Erro ao aplicar retornos personalizados: {e}")
